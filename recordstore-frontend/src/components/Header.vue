<template>
  <header class="bg-gray-200 py-4">
    <div class="container m-auto flex flex-wrap items-center justify-end">
      ...
      <div>
        <router-link to="/" class="link-gray-200 px-2 no-underline" v-if="!signedIn()">Sign in</router-link>
        <router-link to="/signup" class="link-gray-200 px-2 no-underline" v-if="!signedIn()">Sign Up</router-link>
        <router-link to="/records" class="link-gray-200 px-2 no-underline" v-if="signedIn()">Records</router-link>
        <router-link to="/artists" class="link-gray-200 px-2 no-underline" v-if="signedIn()">Artists</router-link>
        <a href="#" @click.prevent="signOut" class="link-gray-200 px-2 no-underline" v-if="signedIn()">Sign out</a>
      </div>
    </div>
  </header>
</template>
